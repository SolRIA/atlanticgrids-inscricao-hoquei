<template>
  <q-page>
    <div class="row justify-center q-ma-xl">
      <img alt="APA" src="~assets/APA.webp" class="col-12" style="width: 200px" />
      <img alt="APACA" src="~assets/APA_APACA.jpg" class="col-12" style="width: 200px" />
    </div>
      ...
      <q-card class="formulario">
            ...
            <template #body="props">
              <q-tr :props="props">
                ...
                <q-td key="nome_pessoa" :props="props">
                  <q-input v-model="props.row.nome_pessoa" outlined @update:model-value="validaPessoa(props.row)" />
                </q-td>

                <!-- cargo -->
                <q-td key="cargo_id" :props="props">
                  <q-select v-model="props.row.cargo_id" :options="cargos" outlined emit-value map-options
                            option-value="id" option-label="nome" @update:model-value="validaPessoa(props.row)" />
                </q-td>

                <!-- accoes -->
                <q-td key="accoes" :props="props" style="width: 30px">
                  <q-btn flat dense color="negative" icon="delete" @click="removePessoa(props.row)" />
                </q-td>
              </q-tr>

              <!-- erros -->
              <q-tr :props="props" v-if="!isNullOrWhiteSpace(props.row.erro)" class="bg-negative">
                <q-td colspan="100%">
        ...
        <q-card-actions>
          <q-btn color="primary" label="Submeter" :loading="saving" :disable="!agreeWithTerms" @click="guardaFormulario"
                 class="full-width" v-if="pessoas.length > 0" />
        </q-card-actions>
      </q-card>
    </div>
  </q-page>
